...
public class MabListener implements Listener {
	...
	public void CornerGrid(int r, int c, int rCount, int cCount, int heightTracker, int stage, Material[] buildList) {
		...
		if (stage == 2) {
			if ((cCount == 0 && rCount < 1) || (rCount == 0 && cCount < 1) || (cCount == 1 && rCount < 2)
					|| (rCount == 1 && cCount < 2) || (cCount == 5 && rCount < 6) || (rCount == 5 && cCount < 6)
					|| (cCount == 11 && rCount < 12) || (rCount == 11 && cCount < 12)) {
				insert(0 + r, 0 + heightTracker, 0 + c, buildList[1], 0, 0, 0);
			} else if (cCount == 15 && rCount < 16) {
				if (rCount % 2 == 0) {
					insert(0 + r, 0 + heightTracker, 0 + c, buildList[1], 0, 0, 0);
				} else {
					insert(0 + r, 0 + heightTracker, 0 + c, buildList[3], 4, 0, 0);
				}
			} else if (rCount == 15 && cCount < 16) {
				if (cCount % 2 == 0) {
					insert(0 + r, 0 + heightTracker, 0 + c, buildList[1], 0, 0, 0);
				} else {
					insert(0 + r, 0 + heightTracker, 0 + c, buildList[3], 1, 0, 0);
